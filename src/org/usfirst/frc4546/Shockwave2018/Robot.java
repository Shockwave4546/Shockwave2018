...
public class Robot extends TimedRobot {
	public static SendableChooser<AutoPosition> autonomousPosition;
	public AutoPosition Left, Center, Right, Disable;
	...
	public void robotInit() {
		...
		/* Position Objects */
		Left = new AutoPosition(1);
		Center = new AutoPosition(2);
		Right = new AutoPosition(3);
		Disable = new AutoPosition(4);

		/* The Position Position */
		autonomousPosition = new SendableChooser<AutoPosition>();
		autonomousPosition.addDefault("Disable", Disable);
	...
	public void autonomousPeriodic() {
		...
		case "Left":
			...
			break;
		case "Center":
			autoselection = "Center";
			break;
		case "Right":
			autoselection = "Right";
			break;
		case "Disable":
			autoselection = "Disable";
			break;
		}
		if (autot < 1) {
	...
	/*
	 * if (autot < 1){ FrontRight.set(-auto * .85); FrontLeft.set(-auto);
	 * BackRight.set(-auto * .85); BackLeft.set(-auto); Timer.delay(4); } else{
	 * FrontRight.set(0); FrontLeft.set(0); BackRight.set(0); BackLeft.set(0); }
	 * 
	 * autot++;
	 */
	...
	public void teleopInit() {
		...
		// Inserts a blank textbox with true or false value (set to false first
		// from Iterative function)
		...
		// Inserts Scaler Modifiers for Each Motor and their +/- on Shuffleboard
		// (Number Input)
	...
	public void teleopPeriodic() {
		...
		if (ToggleSliderValue == false) {
			// Slider for the Left Motor and Right Motor (Gets value from
			// slider)
			...
			// Retrieves Intake (Left & Right), Arm Motor, Slide Motor Scalers
			// from ShuffleBoard.
			...
			// (Front Left Motor Final Value) Displayed on ShuffleBoard
			// Depending on Scaler
			if (FLSlideVal > 0) {
			...
			// (Front Right Motor Final Value) Displayed on ShuffleBoard
			// Depending on Scaler
			if (FRSlideVal > 0) {
			...
			// (Back Left Motor Final Value) Displayed on ShuffleBoard Depending
			// on Scaler
			if (BLSlideVal > 0) {
			...
			// (Back Right Motor Final Value) Displayed on ShuffleBoard
			// Depending on Scaler
			if (BRSlideVal > 0) {
			...
			// (Intake Left Motor Final Value) Displayed on ShuffleBoard
			// Depending on Scaler
			if (ILSlideVal > 0) {
			...
			// (Intake Right Motor Final Value) Displayed on ShuffleBoard
			// Depending on Scaler
			if (IRSlideVal > 0) {
			...
			// (Arm Motor Final Value) Displayed on ShuffleBoard Depending on
			// Scaler
			if (AMSlideVal > 0) {
			...
			// (Slide Motor Final Value) Displayed on ShuffleBoard Depending on
			// Scaler
			if (SMSlideVal > 0) {
		...
		if (ToggleSliderValue == true) {
			...
			if (Button == false && Trigger == false && XboxBump == false) {
				...
				// Intake motor idle
			} else if ((Button == true || XboxBump == true)
					|| (Button == true && XboxBump == true) && Trigger == false) {
			...
			} else if ((FinalFrontLeft == 0) && (FinalFrontRight == 0) && (FinalBackLeft == 0)
					&& (FinalBackRight == 0)) {
				SmartDashboard.putString("Main Direction", "Neutral");
			}

			// Final Motor Value Output Displayed on Shuffleboard (from
			// Controller)
			SmartDashboard.putNumber("FLMotor", FinalFrontLeft);
			SmartDashboard.putNumber("FRMotor", FinalFrontRight);
			SmartDashboard.putNumber("BLMotor", FinalBackLeft);
